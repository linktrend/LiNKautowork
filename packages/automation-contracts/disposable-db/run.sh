...
docker compose -f "$COMPOSE_FILE" -p "$PROJECT_NAME" exec -T postgres psql -v ON_ERROR_STOP=1 -U postgres -d automation_contracts < "$SCRIPT_DIR/provider-plane-verify.sql"
bash "$SCRIPT_DIR/control-restore-rehearsal.sh" "$COMPOSE_FILE" "$PROJECT_NAME"
apply_down "$ROOT_DIR/supabase/migrations/20260813_000001_lautowork_provider_plane.sql"
