...
from scripts.gitops.generated_output_closure import _generate_secret_scan_fixtures
...
ADMITTED_BASE_SHA = "c2272cac3abd92e68f4f81860ddf3bd72b0598ef"
...
class GeneratorConvergenceTests(unittest.TestCase):
    def _stale_tree_repo(self) -> tuple[tempfile.TemporaryDirectory[str], Path, str, bytes]:
        tmp, root = init_repo()
        value = synthetic_value("generator-converge")
        write_tracked(root, "app/runtime.py", "print('reviewed runtime')\n")
        write_tracked(root, "tests/security/test_integrity.py", f'secret = "{value}"\n')
        commit(root, "reviewed application plus synthetic")
        app_bytes = (root / "app/runtime.py").read_bytes()
        write_declaration(
            root,
            declaration(
                candidate_tree="0" * 40,
        ...
        commit(root, "stale candidate tree binding")
        return tmp, root, value, app_bytes

    def test_one_regeneration_converges_and_second_run_is_byte_idempotent(self) -> None:
        tmp, root, _value, app_bytes = self._stale_tree_repo()
        self.addCleanup(tmp.cleanup)
        before = scan_repository(root)
        self.assertFalse(before["ok"])
        self.assertTrue(any(row["kind"] == KIND_STALE for row in before["findings"]))

        first = _generate_secret_scan_fixtures(root)
        self.assertEqual(first, 0)
        git(root, "add", "--", DECLARATION_REL)
        payload = json.loads((root / DECLARATION_REL).read_text(encoding="utf-8"))
        self.assertEqual(payload["candidateTree"], candidate_content_tree(root))
        first_bytes = (root / DECLARATION_REL).read_bytes()

        second = _generate_secret_scan_fixtures(root)
        self.assertEqual(second, 0)
        self.assertEqual(first_bytes, (root / DECLARATION_REL).read_bytes())
        git(root, "add", "--", DECLARATION_REL)
        result = scan_repository(root)
        self.assertTrue(result["ok"], result)
        self.assertEqual(kinds(result), [KIND_APPROVED])
        self.assertEqual((root / "app/runtime.py").read_bytes(), app_bytes)

    def test_stale_real_secret_and_undeclared_inputs_still_fail_closed(self) -> None:
        tmp, root, value, app_bytes = self._stale_tree_repo()
        self.addCleanup(tmp.cleanup)
        _generate_secret_scan_fixtures(root)
        git(root, "add", "--", DECLARATION_REL)
        git(root, "commit", "-qm", "converged declaration")
        self.assertTrue(scan_repository(root)["ok"])

        payload = json.loads((root / DECLARATION_REL).read_text(encoding="utf-8"))
        payload["candidateTree"] = "1" * 40
        write_declaration(root, payload)
        git(root, "commit", "-qm", "stale tree again")
        stale = scan_repository(root)
        self.assertFalse(stale["ok"])
        self.assertTrue(any(row["kind"] == KIND_STALE for row in stale["findings"]))
        ...
        payload["candidateTree"] = candidate_content_tree(root)
        write_declaration(root, payload)
        git(root, "commit", "-qm", "restore binding")

        write_tracked(root, "tests/security/undeclared.py", f'secret = "{synthetic_value("undeclared")}"\n')
        git(root, "commit", "-qm", "undeclared synthetic")
        self.assertEqual(_generate_secret_scan_fixtures(root), 0)
        git(root, "add", "--", DECLARATION_REL)
        git(root, "commit", "-qm", "rebind after undeclared")
        undeclared = scan_repository(root)
        self.assertFalse(undeclared["ok"])
        self.assertTrue(
            any(
                row["kind"] == KIND_CREDENTIAL and row["path"] == "tests/security/undeclared.py"
                for row in undeclared["findings"]
            )
        )
        self.assertFalse(
            any(
                row["kind"] == KIND_APPROVED and row["path"] == "tests/security/undeclared.py"
                for row in undeclared["findings"]
            )
        )

        git(root, "rm", "-q", "tests/security/undeclared.py")
        write_tracked(root, "tests/security/real.py", 'token = "ghp_' + ("A" * 36) + '"\n')
        git(root, "commit", "-qm", "realistic credential")
        self.assertEqual(_generate_secret_scan_fixtures(root), 0)
        git(root, "add", "--", DECLARATION_REL)
        git(root, "commit", "-qm", "rebind after realistic")
        realistic = scan_repository(root)
        self.assertFalse(realistic["ok"])
        self.assertTrue(
            any(
                row["kind"] == KIND_CREDENTIAL and row["path"] == "tests/security/real.py"
                for row in realistic["findings"]
            )
        )
        self.assertFalse(any(row["kind"] == KIND_APPROVED and row["path"] == "tests/security/real.py" for row in realistic["findings"]))
        self.assertEqual((root / "app/runtime.py").read_bytes(), app_bytes)
        self.assertEqual(value, synthetic_value("generator-converge"))

    def test_reviewed_application_files_remain_byte_identical_to_admitted_base(self) -> None:
        changed = git(ROOT, "diff", "--name-only", ADMITTED_BASE_SHA, "HEAD")
        unstaged = git(ROOT, "diff", "--name-only", ADMITTED_BASE_SHA)
        names = {row for row in (changed + "\n" + unstaged).splitlines() if row}
        extra = sorted(names - ALLOWED_REPAIR_PATHS)
