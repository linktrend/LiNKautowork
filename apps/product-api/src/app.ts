...
import { verifyProductApiPaci, type ProductApiPaciIntrospector, type ProductApiPaciJwksProvider } from './paci.js';
import { hasRole, type AuditReservationInput, type ProductApiService } from './service.js';

/** Explicit Product API trust and runtime configuration. */
export type ProductApiEnv = { nodeEnv: 'test' | 'development' | 'production'; issuer: string; audience: string; orgId?: string; paciClientId?: string; testJwtSecret?: string; webhookSecret?: string; maxBodyBytes?: number; paciJwks?: ProductApiPaciJwksProvider; paciIntrospector?: ProductApiPaciIntrospector; publicClientOrigin?: string; operatorConsoleOrigin?: string };
...
/** Production verifier accepts only canonical PACI ES256 tokens with live introspection. */
export async function platformIdentity(env: ProductApiEnv, req: Request): Promise<PlatformIdentity> {
  ...
  if (!env.orgId || !env.paciClientId || !env.paciJwks || !env.paciIntrospector) throw new ProductApiError(503, 'identity_unavailable', 'Live Platform identity conformance is not configured');
  const authorization = req.header('authorization'); if (!authorization?.startsWith('Bearer ')) throw new ProductApiError(401, 'unauthenticated', 'Authentication is required');
  try { return await verifyProductApiPaci({ token: authorization.slice(7), issuer: env.issuer, audience: env.audience, orgId: env.orgId, clientId: env.paciClientId, jwks: env.paciJwks, introspector: env.paciIntrospector }); }
  catch (error) { const message = error instanceof Error ? error.message : ''; if (message === 'jwks_unavailable' || message === 'introspection_unavailable') throw new ProductApiError(503, 'identity_unavailable', 'Live Platform identity verifier is unavailable'); if (message === 'forbidden_token') throw new ProductApiError(403, 'forbidden', 'Platform service or operation is not permitted'); throw new ProductApiError(401, 'invalid_token', 'Authentication token is invalid'); }
}
...
export function createProductApi(env: ProductApiEnv, service: ProductApiService, identityVerifier: (request: Request) => Promise<PlatformIdentity> | PlatformIdentity = (req) => platformIdentity(env, req)) {
  ...
  const identity = (roles: ProductRole[], paciOperation?: 'read') => async (req: Request, _res: Response, next: NextFunction) => {
    try {
      const request = req as AuditedRequest;
      const actor = await identityVerifier(req);
      request.identity = actor;
      const authorised = actor.authentication === 'paci' ? paciOperation !== undefined && actor.permittedOperations?.includes(paciOperation) === true : hasRole(actor.roles, roles);
      if (!authorised) {
  ...
  for (const area of clientAreas) app.get(`/v1/client/${area === 'supportRequests' ? 'support-requests' : area}`, identity(['client_member', 'client_admin'], 'read'), audited(`client.${area}.read`, () => area), async (req, res, next) => { try { const request = req as AuditedRequest; const actor = request.identity as PlatformIdentity; res.json(await service.clientPortal(actor.org_id, area, pageSchema.parse(req.query), request.auditLease)); } catch (error) { next(error); } });
