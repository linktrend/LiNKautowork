...
export type PostgrestConfig = { restUrl: string; runtimeToken: string; apiKey?: string; rpcPath?: string };

/**
 * Makes only named RPC calls. PostgREST derives the role from the signed runtime
 * token; the server supplies the verified browser organisation in its org header.
 * No secret is returned, stored, or sent to a browser.
 */
export function createPostgrestRpc(config: PostgrestConfig): PostgrestRpc {
  if (!/^https?:\/\//.test(config.restUrl)) throw new Error('Product API PostgREST configuration is unavailable');
  // This is a configuration guard, not a signature verifier. PostgREST verifies
  // the signed credential and selects the database role on every request.
  let claims: { role?: string };
  try {
    const parts = config.runtimeToken.split('.');
    if (parts.length !== 3 || parts.some((part) => !part)) throw new Error();
    claims = JSON.parse(Buffer.from(parts[1], 'base64url').toString());
  } catch { throw new Error('Product API requires a scoped runtime JWT'); }
  if (!claims || claims.role !== 'svc_lautowork_product_api') throw new Error('Product API requires svc_lautowork_product_api');
