...
export async function verifyProductApiPaci(input: VerifyProductApiPaciInput): Promise<PlatformIdentity> {
  ...
  if (active.iss !== token.iss || active.aud.length !== token.aud.length || active.aud.some((item, index) => item !== token.aud[index]) || active.sub !== token.sub || active.exp !== token.exp || active.iat !== token.iat || active.jti !== token.jti || active.credential_id !== claims.credentialId || active.runtime_binding_id !== claims.runtimeBindingId || scopes.length !== claims.serviceScopes.length || scopes.some((scope, index) => scope !== claims.serviceScopes[index])) throw new Error('inactive_token');
