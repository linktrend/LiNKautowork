...
const api = createProductApi({ nodeEnv: 'test', issuer: 'local', audience: 'local', testJwtSecret: 'not-used' }, service, () => ({ sub: 'demo-operator', org_id: orgId, roles: ['operator', 'approver'], iss: 'local', aud: 'local', exp: Math.floor(Date.now() / 1000) + 60 }));
