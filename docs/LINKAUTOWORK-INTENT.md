...
- Studio Programs need a shared event language (`aios.*`) and an auditable ledger, not ad-hoc Slack posts.
...
| **Other Programs** (LiNKplatform, LiNKsites, LiNKdeveloper, …) | Consumers of shared org identity (`platform.organizations`) and optional `aios.*` event subjects. They do **not** embed LiNKautowork as a library — they call the gateway / subscribe to events when wired. |
...
3. **Templates** — Live governance templates in `automations/templates/` are the authority for what n8n should run; legacy Program shells that called shelved LiNKaios are archived.
...
5. **Interoperability** — Gateway publishes primary `aios.*` NATS subjects and optional `linkautowork.v1.*` mirrors.
...
| Event interoperability | `aios.*` subjects published; mirrors optional via `ENABLE_INTERNAL_MIRROR_SUBJECTS` |
